%% Infeasible lane change case study
...
sys.add_constraint(diagalways(DiagP('abs(u(t, 1))<=2')));
sys.add_constraint(diagalways(DiagP('abs(u(t, 2))<=1')));
sys.add_constraint(diagalways(DiagP('abs(x(t, 4))<=1')));
...
sys1.add_constraint(diagalways(DiagP('u(t)==[0; -0.25]'))); % If set to zero, feasible

[x, u, y] = sys2.signals();
sys2.add_constraint(DiagP('x(0)==[-0.2; -1.5; pi/2; 0.5]')); % If set to zero, feasible
sys2.add_constraint(diagalways(DiagP('u(t)==[0; 1]')));

[x, u, y] = sys3.signals();
sys3.add_constraint(DiagP('x(0)==[0.2; 1.5; pi/2; 0]'));
sys3.add_constraint(diagalways(DiagP('u(t)==0')));
