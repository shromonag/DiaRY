function [interfacedata,recoverdata,solver,diagnostic,F,Fremoved,ForiginalQuadratics,reordering, ConsFExpandMapping, mapF] = compileinterfacedataWithFeedback(F,aux_obsolete,logdetStruct,h,options,findallsolvers,parametric)
...
if options.expand
    ...
    [F,failure,cause,operators, ConsFExpandMapping, mapF] = expandmodelWithFeedback(F,h,options);
    F = flatten(F);
...
F = lmi(F);
[F_struc,K,KCut,schur_funs,schur_data,schur_variables,reordering] = lmi2sedumistructWithFeedback(F);
