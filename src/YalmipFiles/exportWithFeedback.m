function [model,recoverdata,diagnostic,interfacedata,reordering, ConsFExpandMapping, mapF] = exportWithFeedback(varargin)
...
if ~isempty(F) & any(is(F,'parametric'))
    ...
    [interfacedata,recoverdata,solver,diagnostic,F, Fremoved, ForiginalQuadratics, reordering, ConsFExpandMapping, mapF] = compileinterfacedataWithFeedback(F,[],logdetStruct,h,options,findallsolvers,1);
else
    [interfacedata,recoverdata,solver,diagnostic,F, Fremoved, ForiginalQuadratics, reordering, ConsFExpandMapping, mapF] = compileinterfacedataWithFeedback(F,[],logdetStruct,h,options,findallsolvers);
end
