...
if isa(A,'optimizerWithFeedback') & isa(B,'optimizerWithFeedback')
...
elseif isa(B,'optimizerWithFeedback') & (isa(A,'constraint') | isa(A,'lmi'))
    P = A;
    A = B;
    B = P;
end

P = optimizerWithFeedback([A.F,B],A.h,A.ops,A.input.expression,A.output.expression);
