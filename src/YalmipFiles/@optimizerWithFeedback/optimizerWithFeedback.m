function [sys, aux1, reordering, ConsFExpandMapping, mapF] = optimizerWithFeedback(Constraints,Objective,options,x,u)
...
if ~isempty(Constraints) & any(is(Constraints,'uncertain'))
    ...
    [aux1,aux2,aux3,model,reordering,ConsFExpandMapping, mapF] = exportWithFeedback((x == repmat(pi,nIn*mIn,1))+Constraints,Objective,options,[],[],0);
else
    [aux1,aux2,aux3,model,reordering, ConsFExpandMapping, mapF] = exportWithFeedback((x == repmat(pi,nIn*mIn,1))+Constraints,Objective,options,[],[],0);    
end
...
sys = class(sys,'optimizerWithFeedback');
