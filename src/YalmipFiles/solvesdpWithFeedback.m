function [diagnostic,reordering, ConsFExpandMapping, mapF] = solvesdpWithFeedback(varargin)
...
if nargin>=2
    ...
    elseif isa(varargin{2},'sdpvar') && numel(varargin{2})>1
        % Several objectives
        [diagnostic,reordering, ConsFExpandMapping, mapF] = solvesdp_multipleWithFeedback(varargin{:});
        return
    end
end
...
% ******************************************
[interfacedata,recoverdata,solver,diagnostic,F,Fremoved,ForiginalQuadratics,reordering, ConsFExpandMapping, mapF] = compileinterfacedataWithFeedback(F,[],logdetStruct,h,options,0,solving_parametric);
