function [F,failure,cause,ALREADY_MODELLED, ConsFExpandMapping, mapF] = expandmodelWithFeedback(F,h,options,w)
...
if ~isempty(F)
    ...
    if ~isempty(meta)
        ...
        boundsAlreadySet = 1;
        [F, mapF] = expandmetaWithFeedback(F);
    end
end
...
ConsFExpandMapping = [0 length(F_expand) length(F_expand) 0];
...
while constraint <=length(F) & ~failure
    ...
    final = length(F_expand);
    cumSum = ConsFExpandMapping(end,3);
    ConsFExpandMapping = [ConsFExpandMapping; constraint final-init final-init+cumSum 0];
...
if ~failure
    ...
    if length(REMOVE_THESE_IN_THE_END) > 0
        ...
        R = ismember(getlmiid(F_expand), REMOVE_THESE_IN_THE_END);
        Z = ConsFExpandMapping(:,2);
        len = length(Z);
        K = find(Z);
        consNo = length(R);
        shiftBack = length(K);
        for k=length(R):-1:1
            if(R(k) == 1)
                ...
                while(shiftBack ~= 0 && k <= ConsFExpandMapping(K(shiftBack),3))
                ...
                sub = [zeros(len,1)];
                sub(K(shiftBack)) = 1;
                ConsFExpandMapping(:,2) = ConsFExpandMapping(:,2) - sub;
                
                sub = [zeros(K(shiftBack)-1,1); ones(len - K(shiftBack) + 1,1)];
                ConsFExpandMapping(:,3) = ConsFExpandMapping(:,3) - sub;
                
            end
        end
    end
    ConsFExpandMapping(1,4) = ConsFExpandMapping(end,1) + ConsFExpandMapping(1,2);
    for i = 2: length(ConsFExpandMapping(:,1))
        ConsFExpandMapping(i,4) = ConsFExpandMapping(i-1,4) + ConsFExpandMapping(i,2);
    end
end
