function [F_struc,K,KCut,schur_funs,schur_data,schur_variables, reordering] = lmi2sedumistructWithFeedback(F)
...
for i = 1:length(equ_con)
  ...
  %  [n,m] = size(F.clauses{constraints}.data);
    ...
    top = top+ntimesm;
    K.f = K.f+ntimesm;
    cons = repmat(constraints, 1, ntimesm);
    type = repmat(1, 1, ntimesm);
    temp = [cons; type];
    reordering = [reordering temp];
...
if any_cuts
    for i = 1:length(lin_con)
        ...
        K.l = K.l+ntimesm;
        cons = repmat(constraints, 1, ntimesm);
        type = repmat(2,1,ntimesm);
        temp = [cons; type];
        reordering = [reordering temp];
    end
else
    if length(lin_con)>0
        [F_struc,K,KCut,reordering] = recursive_lp_fix(F,F_struc,K,KCut,lin_con,nvars,4,1, reordering);
    end
end

for i = 1:length(cmp_con)
    constraints = cmp_con(i);
    reordering = [reordering constraints];
    [n,m] = size(F.clauses{constraints}.data);
    ntimesm = n*m; %Just as well pre-calc
...
if length(qdr_con) > 0
    [F_struc,K,KCut,reordering] = recursive_socp_fix(F,F_struc,K,KCut,qdr_con,nvars,8,1, reordering);
end

if length(mqdr_con) > 0
    [F_struc,K,KCut,reordering] = recursive_msocp_fix(F,F_struc,K,KCut,mqdr_con,nvars,inf,1,reordering);
end
...
if any_schur
    [F_struc,K,KCut,schur_funs,schur_data,schur_variables,reordering] = recursive_sdp_fix(F,F_struc,K,KCut,schur_funs,schur_data,schur_variables,sdp_con,nvars,inf,1,reordering);
else
    [F_struc,K,KCut,schur_funs,schur_data,schur_variables,reordering] = recursive_sdp_fix(F,F_struc,K,KCut,schur_funs,schur_data,schur_variables,sdp_con,nvars,8,1,reordering);
end
...
%n = size(F_struc,1)
%m = size(F_struc,2)
%F_strucTemp = zeros([n m])
%k = 1
%for i = 1:size(equ_con)
%    F_strucTemp(equ_con(i), :) = F_struc(k, :);
%    k = k+1;
%end
%for i = 1:size(lin_con)
%    F_strucTemp(lin_con(i), :) = F_struc(k, :);
%    k = k+1;
%end
%F_strucTemp
%F_struc = sparse(F_strucTemp)
...
function [F_struc,K,KCut,reordering] = recursive_lp_fix(F,F_struc,K,KCut,lp_con,nvars,maxnlp,startindex, reordering)

% Check if we should recurse
if length(lp_con)>=2*maxnlp
    % recursing costs, so do 4 in one step
    ind = 1+ceil(length(lp_con)*(0:0.25:1));
    [F_struc1,K,KCut, reordering] = recursive_lp_fix(F,[],K,KCut,lp_con(ind(1):ind(2)-1),nvars,maxnlp,startindex+ind(1)-1, reordering);
    [F_struc2,K,KCut, reordering] = recursive_lp_fix(F,[],K,KCut,lp_con(ind(2):ind(3)-1),nvars,maxnlp,startindex+ind(2)-1, reordering);
    [F_struc3,K,KCut, reordering] = recursive_lp_fix(F,[],K,KCut,lp_con(ind(3):ind(4)-1),nvars,maxnlp,startindex+ind(3)-1, reordering);
    [F_struc4,K,KCut, reordering] = recursive_lp_fix(F,[],K,KCut,lp_con(ind(4):ind(5)-1),nvars,maxnlp,startindex+ind(4)-1, reordering);
    F_struc = [F_struc F_struc1 F_struc2 F_struc3 F_struc4];
    return
elseif length(lp_con)>=maxnlp
    mid = ceil(length(lp_con)/2);
    [F_struc1,K,KCut, reordering] = recursive_lp_fix(F,[],K,KCut,lp_con(1:mid),nvars,maxnlp,startindex, reordering);
    [F_struc2,K,KCut, reordering] = recursive_lp_fix(F,[],K,KCut,lp_con(mid+1:end),nvars,maxnlp,startindex+mid, reordering);
    F_struc = [F_struc F_struc1 F_struc2];
    return
end

oldF_struc = F_struc;
%oldReordering = reordering;
%reordering = [];
F_struc = [];
...
K.l = sum(K.l);
%reordering = [oldReordering reordering];
F_struc = [oldF_struc F_struc];


function [F_struc,K,KCut,schur_funs,schur_data,schur_variables,reordering] = recursive_sdp_fix(F,F_struc,K,KCut,schur_funs,schur_data,schur_variables,sdp_con,nvars,maxnsdp,startindex,reordering)

% Check if we should recurse
if length(sdp_con)>2*maxnsdp
    % recursing costs, so do 4 in one step
    ind = 1+ceil(length(sdp_con)*(0:0.25:1));
    [F_struc1,K,KCut,schur_funs,schur_data,schur_variables,reordering] = recursive_sdp_fix(F,[],K,KCut,schur_funs,schur_data,schur_variables,sdp_con(ind(1):ind(2)-1),nvars,maxnsdp,startindex+ind(1)-1,reordering);
    [F_struc2,K,KCut,schur_funs,schur_data,schur_variables,reordering] = recursive_sdp_fix(F,[],K,KCut,schur_funs,schur_data,schur_variables,sdp_con(ind(2):ind(3)-1),nvars,maxnsdp,startindex+ind(2)-1,reordering);
    [F_struc3,K,KCut,schur_funs,schur_data,schur_variables,reordering] = recursive_sdp_fix(F,[],K,KCut,schur_funs,schur_data,schur_variables,sdp_con(ind(3):ind(4)-1),nvars,maxnsdp,startindex+ind(3)-1,reordering);
    [F_struc4,K,KCut,schur_funs,schur_data,schur_variables,reordering] = recursive_sdp_fix(F,[],K,KCut,schur_funs,schur_data,schur_variables,sdp_con(ind(4):ind(5)-1),nvars,maxnsdp,startindex+ind(4)-1,reordering);
    F_struc = [F_struc F_struc1 F_struc2 F_struc3 F_struc4];
    return
elseif length(sdp_con)>maxnsdp
    mid = ceil(length(sdp_con)/2);
    [F_struc1,K,KCut,schur_funs,schur_data,schur_variables,reordering] = recursive_sdp_fix(F,[],K,KCut,schur_funs,schur_data,schur_variables,sdp_con(1:mid),nvars,maxnsdp,startindex,reordering);
    [F_struc2,K,KCut,schur_funs,schur_data,schur_variables,reordering] = recursive_sdp_fix(F,[],K,KCut,schur_funs,schur_data,schur_variables,sdp_con(mid+1:end),nvars,maxnsdp,startindex+mid,reordering);
    F_struc = [F_struc F_struc1 F_struc2];
    return
end
...
function [F_struc,K,KCut,reordering] = recursive_socp_fix(F,F_struc,K,KCut,qdr_con,nvars,maxnsocp,startindex,reordering);

% Check if we should recurse
if length(qdr_con)>2*maxnsocp
    % recursing costs, so do 4 in one step
    ind = 1+ceil(length(qdr_con)*(0:0.25:1));
    [F_struc1,K,KCut,reordering] = recursive_socp_fix(F,[],K,KCut,qdr_con(ind(1):ind(2)-1),nvars,maxnsocp,startindex+ind(1)-1,reordering);
    [F_struc2,K,KCut,reordering] = recursive_socp_fix(F,[],K,KCut,qdr_con(ind(2):ind(3)-1),nvars,maxnsocp,startindex+ind(2)-1,reordering);
    [F_struc3,K,KCut,reordering] = recursive_socp_fix(F,[],K,KCut,qdr_con(ind(3):ind(4)-1),nvars,maxnsocp,startindex+ind(3)-1,reordering);
    [F_struc4,K,KCut,reordering] = recursive_socp_fix(F,[],K,KCut,qdr_con(ind(4):ind(5)-1),nvars,maxnsocp,startindex+ind(4)-1,reorderin);
    F_struc = [F_struc F_struc1 F_struc2 F_struc3 F_struc4];
    return
elseif length(qdr_con)>maxnsocp
    mid = ceil(length(qdr_con)/2);
    [F_struc1,K,KCut,reordering] = recursive_socp_fix(F,[],K,KCut,qdr_con(1:mid),nvars,maxnsocp,startindex,reordering);
    [F_struc2,K,KCut,reordering] = recursive_socp_fix(F,[],K,KCut,qdr_con(mid+1:end),nvars,maxnsocp,startindex+mid,reordering);
    F_struc = [F_struc  F_struc1  F_struc2];
    return
end
...
function [F_struc,K,KCut,reordering] = recursive_msocp_fix(F,F_struc,K,KCut,qdr_con,nvars,maxnsocp,startindex,reordering);
