%% This function creates a list of constraints which are need to be changed along with their maximal slacks

function status = UpdateMaxSlackMILP(STLparseTrees, DiagnosedPredicates)
    constraintsToChange = [];
    ...
    fclose(fileID);
    [status, result] = unix('python ./PythonFiles/UpdateMaxSlackMILP.py');
end